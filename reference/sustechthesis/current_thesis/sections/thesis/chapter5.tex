...
相对于 Android 端侧直接运行 7B 目标模型，跨设备投机解码的吞吐率由 7.232 token/s 提升到 13.996 token/s，相对提升约 93.5\%。该结果表明，在端侧直接运行较大目标模型成本较高的情况下，使用轻量草稿模型参与生成、并将目标验证放到桌面端执行，能够明显降低长输出任务的等待时间。
...
本章在统一模型组合、长输出任务和稳态墙钟时间口径下，对端侧本地目标模型推理、云侧本地单进程投机解码、云侧本地双进程投机解码和 Android--桌面端跨设备投机解码进行了比较。实验结果表明，云侧本地双进程方案能够保持接近单进程方案的吞吐率，跨设备方案能够保持接近云侧本地双进程方案的吞吐率，并显著快于 Android 端侧直接运行 7B 目标模型。
